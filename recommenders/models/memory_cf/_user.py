...
UserCFType = TypeVar("UserCFType", bound="UserCF")


class UserCF(BaseMemoryCF):
    ...
    def predict(
        ...
        mask = self.mask.loc[:, item]
        ratings = self.df.loc[mask, item]
        weights = self.similarity.loc[user, mask]
